# 基于国密算法的安全数据传输与身份认证系统

本项目是一个大学生课程设计，实现了一套基于国密算法的安全数据传输演示系统。系统以单一 envelope 报文为核心，模拟发送端生成安全报文、接收端验证并恢复明文的完整流程。

## 项目任务说明

**项目名称**：基于国产商用密码算法的安全数据传输与身份认证系统

**系统核心功能**：设计一个基于国产商用密码算法的安全数据传输与身份认证系统，实现电子文件在发送端与接收端之间的安全传输。系统综合使用 SM2、SM3、SM4、ZUC、SM9 等国密算法，确保传输文件的机密性、完整性、真实性与身份可信性，并通过可视化界面展示加密、签名、完整性校验、解密和验证等关键流程及中间结果。

**要求**：

（1）基于 Python 与 PySide6 框架设计并开发一个桌面端安全数据传输与身份认证系统，系统需要具备良好的可视化效果、可维护性和扩展性。

（2）系统采用发送端与接收端双窗口交互模式，发送端负责文件选择、算法配置、加密封装和签名，接收端负责信封解析、身份验证、完整性校验、文件解密和结果展示。

（3）基于 SM2 算法实现会话秘密封装与解封，基于 SM3 算法实现消息摘要、HMAC 完整性校验与密钥派生，基于 SM4 或 ZUC 算法实现文件内容加密，基于 SM9 算法实现身份签名与签名验证，确保数据来源可信、内容未被篡改。

（4）系统前后端逻辑相对分离，前端通过简洁、美观、易操作的 PySide6 图形界面展示安全传输流程，后端通过核心协议模块完成加密、签名、验证、解密等业务逻辑，并向前端返回关键中间结果。

（5）系统能够生成标准化的安全传输信封文件，记录协议版本、算法套件、发送方身份、接收方身份、文件名、会话标识、密文、认证标签和数字签名等信息。

（6）系统提供攻击模拟功能，支持对密文、IV/Nonce、接收方身份、文件名和 SM9 签名等内容进行篡改测试，并在接收端直观展示签名验证、完整性验证、摘要比对和文件类型识别结果。

（7）系统支持对电子文件进行保密传输与恢复保存，能够对恢复文件进行摘要校验和文件类型一致性检查，验证传输过程的正确性与安全性。

## 功能概览

- SM2：封装会话秘密。
- HKDF-SM3：派生加密密钥、IV/nonce 和完整性密钥。
- SM4：支持 CBC、CTR、GCM 三种加密模式。
- ZUC-128：提供流加密路径。
- HMAC-SM3：为非 GCM 模式提供完整性保护。
- SM9：对完整协议 transcript 进行数字签名与验签。
- 任意文件传输：明文按原始字节加密与恢复（图片/PDF/压缩包等二进制文件无损往返），恢复文件保留原扩展名。
- 文件类型识别：信封 header 携带受签名与 HMAC 双保护的原始文件名；接收端另以 magic bytes 嗅探解密内容真实类型，与声称类型对比展示。
- CLI：提供环境检查、演示、发送、基准测试和测试入口。
- GUI：基于 PySide6 的**双窗口**图形界面——发送端主窗口 + 自动弹出的接收端验证窗口，以时间线卡片逐步展示协议中间过程，内置攻击模拟与深浅主题切换。

## 协议报文

发送端输出一个统一报文：

```text
artifacts/message.json
```

报文结构：

```text
Envelope {
  header: {
    version,
    suite_id,
    cipher,
    mode,
    kex_mode,
    sender_id,
    receiver_id,
    filename,
    session_id,
    timestamp,
    seq,
    integrity_algo
  },
  algo_meta,
  wrapped_secret_b64,
  nonce_or_iv_b64,
  ciphertext_b64,
  auth_tag_b64,
  signature_b64
}
```

KDF 上下文绑定：

```text
session_id | sender_id | receiver_id | suite_id
```

SM9 签名 transcript：

```text
header || wrapped_secret || nonce_or_iv || ciphertext || auth_tag
```

## 项目结构

```text
core/
  protocol.py       协议常量
  sender.py         发送端流程
  receiver.py       接收端流程
  workflow.py       一键完整流程
  benchmark.py      性能测试

crypto/
  sm2_kex_or_wrap.py  SM2 会话秘密封装
  sm3_integrity.py    SM3/HMAC/transcript 工具
  sm4_adapter.py      SM4 CBC/CTR/GCM
  sm9_signature.py    SM9 签名与验签
  zuc_adapter.py      ZUC-128
  kdf_utils.py        HKDF-SM3 派生
  metadata_utils.py   envelope 读写

gui/
  run.py              GUI 入口
  main_window.py      发送端主窗口（顶栏 + 左导航 + 时间线 + 日志台，发送完成后弹出接收端）
  receiver_window.py  接收端独立窗口（验证时间线 + 日志台）
  timeline_view.py    步骤卡片时间线容器
  step_card.py        单步骤卡片（内容驱动渲染路由）
  data_widgets.py     卡片内数据组件（键值格、长串截断、验证胶囊、结论横幅、对比块）
  workers.py          后台线程（含攻击模拟篡改逻辑）
  log_widget.py       语法高亮日志台
  styles.py           QSS 主题系统
  effects.py          动效与自绘组件
  elided_label.py     省略号标题标签
  frameless_resize.py 无边框窗口边缘缩放光标支持

config/
  style.json        配色主题数据（「默认」主题的 light/dark 两套）

tests/
  test_crypto.py      回归测试
  test_build_spec.py  打包配置守护测试

cli.py                命令行入口
plain.txt             示例明文
```

## 环境准备

建议使用 Python 3.10 或更高版本。可以使用系统 Python、虚拟环境或 conda 环境。

使用普通 Python 环境：

```bash
python -m pip install -r requirements.txt
```

使用 venv：

```bash
python -m venv .venv
.venv\Scripts\activate
python -m pip install -r requirements.txt
```

使用 conda：

```bash
conda create -n guomi-crypto python=3.12
conda activate guomi-crypto
python -m pip install -r requirements.txt
```

SM9 依赖 GmSSL 原生动态库。如果环境中缺少原生库，SM9 完整流程无法运行。可以先执行环境检查命令确认本机支持情况。

## 命令行使用

环境检查：

```bash
python cli.py inspect-env
```

完整演示：

```bash
python cli.py demo
python cli.py demo --cipher sm4 --mode gcm
python cli.py demo --cipher zuc
```

生成 envelope：

```bash
python cli.py send --cipher sm4 --mode gcm --in plain.txt
```

性能测试：

```bash
python cli.py benchmark
```

运行测试：

```bash
python -m pytest tests/ -q
```

## GUI 使用

```bash
python gui/run.py
```

GUI 采用**发送端 + 接收端双窗口**架构（同进程视觉分离），默认浅色模式启动。

发送端为主控窗口，左侧导航两个功能页：

- 演示：选择算法后点击「⚡ 启动演示」，时间线卡片逐步展示发送端 3 个步骤（SM9 主密钥、SM2 密钥对、加密·签名·封装）。
- 环境：查看运行环境与各算法组件状态。

发送端步骤完成后**自动弹出接收端窗口**（滑入 + 淡入动画），展示 6 个验证步骤：

1. 加载信封 · SM2 解封会话秘密
2. SM9 签名验证（pass/fail 胶囊）
3. HMAC 完整性验证（信封声称值 vs 独立计算值上下对比块；GCM 模式显示认证标签胶囊）
4. SM3 摘要比对（同样的对比块）
5. 文件类型识别（信封声称类型 vs magic bytes 内容检测类型对比块）
6. 最终结论（通过/失败横幅 + 恢复文件路径）

发送端顶栏控件：

- 算法选择器：zuc / sm4-gcm / sm4-cbc / sm4-ctr。
- 攻击模拟：在发送后、接收前真实篡改信封，触发接收端的真实校验失败。可选「正常传输 / 篡改密文 / 篡改 IV·Nonce / 伪造接收方 ID / 篡改文件名 / 伪造 SM9 签名」。
- 主题按钮 ◑：深色/浅色一键切换，整窗截图淡出过渡，并同步联动接收端窗口。

两个窗口均为无边框设计，支持标题栏拖动、边缘拖拽缩放与缩放光标提示。右侧日志台实时输出带语法高亮和时间戳的执行日志。

### 攻击模拟说明

攻击模拟用于演示国密协议的分层安全保证。选择某种攻击后，`WorkflowWorker` 会在发送端写出 `message.json` 之后、接收端读取之前篡改对应字段，接收端跑的是真实的 SM2 / HMAC-SM3 / SM9 校验，失败由密码学产生而非伪造。典型结果：

| 攻击 | SM9 签名 | 完整性 | 摘要 |
|------|----------|--------|------|
| 正常传输 | 通过 | 通过 | 通过 |
| 篡改密文 | 失败 | 失败 | 失败 |
| 篡改 IV/Nonce | 失败 | 失败 | 失败 |
| 伪造接收方 ID | 失败 | 失败 | 失败 |
| 篡改文件名 | 失败 | 失败 | 失败 |
| 伪造 SM9 签名 | 失败 | 通过 | 失败 |

「伪造 SM9 签名」一行体现了完整性（数据未被篡改）与来源认证（确为发送方所签）是两个独立的安全属性。

## 输出文件

运行后主要输出在 `artifacts/`：

```text
message.json      统一协议报文
receiver_pri.txt  演示用接收方 SM2 私钥
receiver_pub.txt  演示用接收方 SM2 公钥
recovered.*       接收端恢复出的原始文件（按原始字节写出，保留原扩展名）
benchmark.md      性能测试报告
benchmark.csv     性能测试数据
```

## 测试覆盖

测试包括：

- SM2 封装和解封装。
- HKDF-SM3 上下文绑定。
- HMAC-SM3 完整性验证。
- SM4 CBC/CTR/GCM 加解密。
- ZUC-128 加解密。
- envelope 完整发送接收流程。
- 二进制明文（含非 UTF-8 字节）逐字节无损往返（SM4-CBC/GCM、ZUC）。
- 篡改 ciphertext、nonce_or_iv、receiver_id、filename 后验证失败。
- 文件类型嗅探与扩展名一致性检查。
- 打包配置守护（新增 GUI 模块与 shiboken6 运行库的收录检查）。

当前验证结果：

```text
48 passed
```

## 说明

当前 Windows 环境中的 `gmssl-python` SM9 主密钥对象不能稳定跨进程序列化，因此推荐使用 `demo` 或 GUI 完整演示流程。它们在同一进程中持有 SM9 主密钥，可以完成签名和验签。

使用「伪造 SM9 签名」攻击模拟时，GmSSL 原生库会向终端打印若干 C 层诊断信息（如 `sm9_point_is_on_curve`）。这是底层库对畸形签名的正常报错，不影响 GUI 运行，验签会正确返回失败。





